...
// ! Chart function
const drawD3LineAndBar = ({
  selector = '#revenue-chart',
  data = REVENUE_DATA,
  xDomain = REVENUE_DATA.map(d => d.shortMonth)
}) => {
  ...
  svg
    .append('g')
    .call(
      d3.axisLeft(y).ticks(y_axis_tick_count).tickFormat(format_ticks_use_k)
    );
  ...
  svg
    ...
    .append('rect')
    .attr('class', d => {
      if (d.isProjection) return 'bar fill-light';
      if (d.actual === d.budget) return 'bar fill-purple';
